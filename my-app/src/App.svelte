<script>
  ...
  function loading(){
    setInterval(() => {
      rotate_deg += 1
      document.getElementById('rotate').style = `transform:rotate(${rotate_deg}deg);`
    }, 10);
  }
...
<main class="max-w-md w-screen h-screen">
  ...
  >
    ...
    >
      ...
      {#if !opening_manu}
        <div class="mb-2 mx-6 text-black">收件匣</div>
      {/if}
    </div>
    <div class="flex mt-2 p-4 hover:bg-green-100 rounded-lg"
    ...
    >
      ...
      {#if !opening_manu}
        <div class="mb-2 mx-6 text-black">新信息</div>
      {/if}
    </div>
    <div class="flex mt-2 p-4 hover:bg-green-100 rounded-lg"
  ...
  <div class="flex inset-0 space-x-2 bg-gray-400 absolute h-screen w-screen flex justify-center items-center">
    <div class="text-xl">loading</div>
      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6" id="rotate">
        <path stroke-linecap="round" stroke-linejoin="round" d="M16.023 9.348h4.992v-.001M2.985 19.644v-4.992m0 0h4.992m-4.993 0l3.181 3.183a8.25 8.25 0 0013.803-3.7M4.031 9.865a8.25 8.25 0 0113.803-3.7l3.181 3.182m0-4.991v4.99" />
      </svg>   
  </div>
  {/if}
